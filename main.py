...
def home():
    ...
    if isLogin():
        ...
        request.cookies.get("userName"), author=ans[3], title=ans[0], like=ans[2])
    con.close()
    return render_template('home.htm', author=ans[3], title=ans[0], like=ans[2])
...
def like(title):
    ...
    if title in likes:
        ...
        cur.execute(sql, (likes+title, request.cookies.get("userName")))
        con.commit()
    con.close()
    return redirect(f'/postpage/{title}')
